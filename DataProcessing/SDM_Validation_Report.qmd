...
if (file.exists(metrics_clean_file) && file.exists(metrics_rg_file)) {
  ...
  m_cv_clean <- compute_detailed_metrics(cv_clean$cots_problem, cv_clean$.pred_1, cv_clean$.pred_class, "Clean (12 Predictors)", "Cross-Validation (Out-of-Fold)")
  m_cv_rg    <- compute_detailed_metrics(cv_rg$cots_problem, cv_rg$.pred_1, cv_rg$.pred_class, "Reef Guide (17 Predictors)", "Cross-Validation (Out-of-Fold)")

  cat("### 1.1 Comparative Model Performance: Spatial Block Cross-Validation\n\n")
  cv_table <- bind_rows(m_cv_clean, m_cv_rg)
  ...
  if (file.exists("outputs/model_performance_metrics_clean.rds")) {
    ...
    cat("### 1.2 Regional Validation Performance (Clean Model Out-of-Fold)\n\n")
    print(kable(m_clean_metrics$By_Zone %>% arrange(ManagementZone), digits = 3, caption = "Clean Model Performance by Management Zone"))
    cat("\n\n")
  }
} else {
...
## 2b. Cull Dive Validation (ROC & AUC across All 6 Model Variants)

Cull dive data provides a highly reliable validation target because divers directly count COTS at each site. We extract spatial predictions at each cull dive location (Jan 2025 onwards across top 30 evaluated reefs) and compare model discrimination and classification performance across all six model configurations:
1. **RHIS XGBoost Clean (12-pred)**
2. **RHIS XGBoost Reef Guide (17-pred)**
3. **Manta Tow XGBoost Clean (12-pred)**
4. **Manta Tow XGBoost Reef Guide (17-pred)**
5. **MaxEnt Clean (12-pred, 10k bg)**
6. **MaxEnt Reef Guide (17-pred, 10k bg)**
...
cull_val_df <- tibble(
  ...
  rhis_clean  = if(!is.null(prob_year2025))    as.numeric(terra::extract(prob_year2025, v_cull_pts)[[2]]) else NA_real_,
  rhis_rg     = if(!is.null(prob_2025_rg))     as.numeric(terra::extract(prob_2025_rg, v_cull_pts)[[2]]) else NA_real_,
...
roc_models <- list(
  "RHIS XGBoost Reef Guide (17-pred)" = pull(cull_val_df, rhis_rg),
  "RHIS XGBoost Clean (12-pred)"      = pull(cull_val_df, rhis_clean),
...
p_roc <- ggplot(roc_df_all, aes(x = FPR, y = TPR, color = Model)) +
  ...
  labs(
    title = "ROC Curves: 6 Model Variants vs Cull Dive COTS Presence (2025)",
...
model_cols <- list(
  "RHIS XGBoost Reef Guide (17-pred)"  = "rhis_rg",
  "RHIS XGBoost Clean (12-pred)"       = "rhis_clean",
...
print(kable(cull_perf_table, caption = "Independent Cull Validation Performance Metrics across All 6 SDM Variants (Threshold = Median Probability)"))
cat("\n\n")

# --- Optimal thresholds (Youden's J) ---
roc_rhis_rg <- pROC::roc(response = y_true, predictor = pull(cull_val_df, rhis_rg), quiet = TRUE)
opt_rhis_rg <- coords(roc_rhis_rg, "best", best.method = "youden", ret = c("threshold", "sensitivity", "specificity"))

cat("Optimal Threshold (Youden's J) for RHIS XGBoost Reef Guide Model:", round(opt_rhis_rg$threshold, 4),
    "  Sensitivity:", round(opt_rhis_rg$sensitivity, 3),
    "  Specificity:", round(opt_rhis_rg$specificity, 3), "\n\n")

# --- Confusion Matrices at P88 and P95 thresholds ---
thresh_rg_p88 <- quantile(cull_val_df$rhis_rg, 0.88, na.rm = TRUE)
thresh_rg_p95 <- quantile(cull_val_df$rhis_rg, 0.95, na.rm = TRUE)
...
p_cm1 <- plot_cull_cm(cull_val_df$rhis_rg, cull_val_df$observed_presence == 1, 
                       thresh_rg_p88, paste0("RHIS ReefGuide @ P88 (", round(thresh_rg_p88, 4), ")"))
p_cm2 <- plot_cull_cm(cull_val_df$rhis_rg, cull_val_df$observed_presence == 1, 
                       thresh_rg_p95, paste0("RHIS ReefGuide @ P95 (", round(thresh_rg_p95, 4), ")"))
p_cm3 <- plot_cull_cm(cull_val_df$rhis_rg, cull_val_df$observed_presence == 1, 
                       opt_rhis_rg$threshold, paste0("RHIS ReefGuide @ Optimal (", round(opt_rhis_rg$threshold, 4), ")"))
