...
if (file.exists(metrics_clean_file)) {
  ...
  print(kable(m_clean$By_Zone %>% arrange(ManagementZone), digits = 3, caption = "Clean Model Performance by Management Zone"))
  cat("\n\n")
} else {
  cat("*Model performance metrics file not found. Ensure `fit_predict_cots_sdm.R` has been run recently.*")
}
```
...
cull_pts_2025 <- st_as_sf(
  cull_raw %>% filter(ReefName %in% reef_select_eval, as.Date(SurveyDate) >= as.Date("2025-01-01")),
...
cull_extract_agn <- terra::extract(prob_agnostic, terra::vect(cull_pts_2025))
...
val_col_agn <- setdiff(names(cull_extract_agn), "ID")[1]

# Extract RG model probability (if available)
cull_extract_rg <- if(!is.null(prob_2025_rg)) terra::extract(prob_2025_rg, terra::vect(cull_pts_2025)) else NULL
val_col_rg <- if(!is.null(cull_extract_rg)) setdiff(names(cull_extract_rg), "ID")[1] else NULL
...
cull_val_df <- data.frame(
  ...
  prob_agnostic = cull_extract_agn[[val_col_agn]],
...
roc_cull_max  <- if("prob_maxent" %in% names(cull_val_df)) roc(cull_val_df$observed_presence, cull_val_df$prob_maxent, quiet = TRUE) else NULL
roc_cull_rg   <- if("prob_rg" %in% names(cull_val_df)) roc(cull_val_df$observed_presence, cull_val_df$prob_rg, quiet = TRUE) else NULL
...
opt_2025 <- coords(roc_cull_2025, "best", best.method = "youden", ret = c("threshold", "sensitivity", "specificity"))
opt_agn  <- coords(roc_cull_agn,  "best", best.method = "youden", ret = c("threshold", "sensitivity", "specificity"))
