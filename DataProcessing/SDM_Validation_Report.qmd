...
# --- Configuration ---
predict_agnostic_path       <- "outputs/COTS_prob_0.02_cpue_agnostic_withRG.tif"
predict_agnostic_clean_path <- "outputs/COTS_prob_0.02_cpue_agnostic_clean.tif"
...
predict_ensemble_2025_path  <- "outputs/COTS_prob_ensemble_2025.tif"
predict_ensemble_agn_path   <- "outputs/COTS_prob_ensemble_agnostic.tif"
...
prob_agnostic_clean <- if(file.exists(predict_agnostic_clean_path)) terra::rast(predict_agnostic_clean_path) else NULL
prob_agnostic_rg    <- if(file.exists(predict_agnostic_path))       terra::rast(predict_agnostic_path) else NULL
prob_agnostic       <- if(!is.null(prob_agnostic_rg)) prob_agnostic_rg else prob_agnostic_clean
...
prob_ensemble_2025  <- if(file.exists(predict_ensemble_2025_path)) terra::rast(predict_ensemble_2025_path) else NULL
prob_ensemble_agn   <- if(file.exists(predict_ensemble_agn_path))  terra::rast(predict_ensemble_agn_path) else NULL
prob_ensemble       <- if(!is.null(prob_ensemble_2025)) prob_ensemble_2025 else if(file.exists(predict_ensemble_path)) terra::rast(predict_ensemble_path) else NULL
...
Cull dive data provides a highly reliable validation target because divers directly count COTS at each site. We evaluate model discrimination and classification performance across **all 23,439 historical cull dives (2018–2025)** and the **2025 operational dive subset**, comparing all base model variants and the dual ensemble rasters:
1. **Performance-Weighted Ensemble 2025 (`COTS_prob_ensemble_2025.tif`)**
2. **Outbreak Persistence Ensemble Agnostic (`COTS_prob_ensemble_agnostic.tif`)**
3. **Cull Dive XGBoost 2025 Reef Guide (17-pred)**
4. **Cull Dive XGBoost 2025 Clean (12-pred)**
5. **Cull Dive XGBoost Agnostic Reef Guide (17-pred)**
6. **Cull Dive XGBoost Agnostic Clean (12-pred)**
7. **MaxEnt Reef Guide (17-pred, 10k bg)**
8. **MaxEnt Clean (12-pred, 10k bg)**
9. **Manta Tow XGBoost Clean (12-pred)**
10. **Manta Tow XGBoost Reef Guide (17-pred)**

```{r step_a_cull_roc, fig.width=12, fig.height=10}
# --- PART 1: ALL HISTORICAL CULL DIVES (2018-2025) ---
cull_pts_all_sf <- st_as_sf(cull_raw, coords = c("Longitude", "Latitude"), crs = 4326) %>%
  st_transform(crs = crs(prob_year2025))

v_cull_all_pts <- terra::vect(cull_pts_all_sf)

cull_val_all_df <- tibble(
  observed_presence = as.integer(pull(cull_raw, CPUE) >= 0.02),
  CPUE              = pull(cull_raw, CPUE),
  ens_2025     = if(!is.null(prob_ensemble_2025)) as.numeric(terra::extract(prob_ensemble_2025, v_cull_all_pts)[[2]]) else NA_real_,
  ens_agn      = if(!is.null(prob_ensemble_agn))  as.numeric(terra::extract(prob_ensemble_agn, v_cull_all_pts)[[2]]) else NA_real_,
  cull_2025_rg = if(!is.null(prob_2025_rg))       as.numeric(terra::extract(prob_2025_rg, v_cull_all_pts)[[2]]) else NA_real_,
  cull_2025_cl = if(!is.null(prob_year2025))      as.numeric(terra::extract(prob_year2025, v_cull_all_pts)[[2]]) else NA_real_,
  cull_agn_rg  = if(!is.null(prob_agnostic_rg))   as.numeric(terra::extract(prob_agnostic_rg, v_cull_all_pts)[[2]]) else NA_real_,
  cull_agn_cl  = if(!is.null(prob_agnostic_clean)) as.numeric(terra::extract(prob_agnostic_clean, v_cull_all_pts)[[2]]) else NA_real_,
  manta_clean  = if(!is.null(prob_manta_clean))   as.numeric(terra::extract(prob_manta_clean, v_cull_all_pts)[[2]]) else NA_real_,
  manta_rg     = if(!is.null(prob_manta_rg))      as.numeric(terra::extract(prob_manta_rg, v_cull_all_pts)[[2]]) else NA_real_,
  maxent_clean = if(!is.null(prob_maxent))        as.numeric(terra::extract(prob_maxent, v_cull_all_pts)[[2]]) else NA_real_,
  maxent_rg    = if(!is.null(prob_maxent_rg))     as.numeric(terra::extract(prob_maxent_rg, v_cull_all_pts)[[2]]) else NA_real_
) %>% drop_na()

cat("### 2b.1 Performance across ALL Historical Cull Dives (2018–2025: n =", nrow(cull_val_all_df), "dives)\n\n")

y_true_all <- pull(cull_val_all_df, observed_presence)

roc_models_all <- list(
  "Ensemble 2025 (Weighted)"            = pull(cull_val_all_df, ens_2025),
  "Ensemble Agnostic (Persistence)"     = pull(cull_val_all_df, ens_agn),
  "Cull XGBoost 2025 Reef Guide (17)"   = pull(cull_val_all_df, cull_2025_rg),
  "Cull XGBoost Agnostic Reef Guide(17)"= pull(cull_val_all_df, cull_agn_rg),
  "Cull XGBoost 2025 Clean (12)"        = pull(cull_val_all_df, cull_2025_cl),
  "Cull XGBoost Agnostic Clean (12)"     = pull(cull_val_all_df, cull_agn_cl),
  "MaxEnt Reef Guide (17, 10k)"         = pull(cull_val_all_df, maxent_rg),
  "MaxEnt Clean (12, 10k)"              = pull(cull_val_all_df, maxent_clean),
  "Manta XGBoost Clean (12)"            = pull(cull_val_all_df, manta_clean)
)

roc_df_all_list <- list()
for (m_label in names(roc_models_all)) {
  p_vec <- roc_models_all[[m_label]]
  if (!all(is.na(p_vec))) {
    roc_obj <- pROC::roc(response = y_true_all, predictor = p_vec, quiet = TRUE)
    auc_val <- as.numeric(pROC::auc(roc_obj))
    roc_df_all_list[[m_label]] <- data.frame(
...
p_roc_all <- ggplot(bind_rows(roc_df_all_list), aes(x = FPR, y = TPR, color = Model)) +
  geom_line(linewidth = 1.2) +
  geom_abline(linetype = "dashed", color = "grey50") +
  theme_minimal(base_size = 13) +
  labs(
    title = "ROC Curves: All Historical Cull Dives (2018–2025)",
    subtitle = paste0("n = ", nrow(cull_val_all_df), " historical cull dives across GBR"),
...
# Detailed metric table across all historical dives
cull_all_metrics_list <- list()
for (m_name in names(roc_models_all)) {
  p_vec <- roc_models_all[[m_name]]
  if (!all(is.na(p_vec))) {
    c_thresh <- median(p_vec, na.rm = TRUE)
    cull_all_metrics_list[[m_name]] <- compute_detailed_metrics(
      truth_vec = y_true_all,
      prob_vec  = p_vec,
      model_name = m_name,
      dataset_name = "All Historical Cull Dives (2018-2025)",
      threshold = c_thresh
    )
  }
}

print(kable(bind_rows(cull_all_metrics_list), caption = "Performance Metrics across ALL Historical Cull Dives (2018–2025, Threshold = Median Probability)"))
cat("\n\n")

# --- PART 2: OPERATIONAL 2025 CULL DIVES ---
...
cull_val_df <- tibble(
  ...
  cull_agn_rg = if(!is.null(prob_agnostic_rg)) as.numeric(terra::extract(prob_agnostic_rg, v_cull_pts)[[2]]) else NA_real_,
...
cat("### 2b.2 Performance across Operational 2025 Cull Dives (n =", nrow(cull_val_df), "dives)\n\n")
...
roc_models <- list(
  ...
  "Cull Dive XGBoost Agnostic RG (17)"   = pull(cull_val_df, cull_agn_rg),
...
p_roc <- ggplot(bind_rows(roc_df_list), aes(x = FPR, y = TPR, color = Model)) +
  geom_line(linewidth = 1.2) +
  geom_abline(linetype = "dashed", color = "grey50") +
  theme_minimal(base_size = 13) +
  labs(
    title = "ROC Curves: Operational 2025 Cull Dives",
    subtitle = paste0("n = ", nrow(cull_val_df), " independent cull dives from top 30 reefs (2025)"),
    x = "False Positive Rate (1 - Specificity)",
    y = "True Positive Rate (Sensitivity)",
    color = "Model Variant"
  ) +
...
for (m_name in names(roc_models)) {
  p_vec <- roc_models[[m_name]]
  if (!all(is.na(p_vec))) {
    c_thresh <- median(p_vec, na.rm = TRUE)
    cull_metrics_list[[m_name]] <- compute_detailed_metrics(
      ...
      prob_vec  = p_vec,
      model_name = m_name,
      dataset_name = "2025 Operational Cull Validation",
      threshold = c_thresh
    )
  }
}

print(kable(bind_rows(cull_metrics_list), caption = "Operational 2025 Cull Validation Performance Metrics (Threshold = Median Probability)"))
cat("\n\n")
