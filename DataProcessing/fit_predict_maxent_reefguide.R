############################################################
# fit_predict_maxent_reefguide.R
# Fit a MaxEnt (presence-only) SDM for COTS using maxnet,
# with ENMeval hyperparameter tuning using all 17 predictors
# (12 base environmental/coral + 5 Reef Guide layers).
...
predictors <- terra::rast(predict_stack_full)

# Standardize layer names
names(predictors)[1:17] <- c(
  "BPI", "EAST", "HCU", "NORTH", "SLO", "SVF", "VCU", "VRM", "DEM", "AhyaD", "Aspat", "Aten",
  "RG_bathy", "RG_slope", "RG_turbid", "RG_waves_Hs", "RG_waves_Tp"
)

pred_names <- names(predictors)
print(paste("Predictor layers (17 layers):", paste(pred_names, collapse = ", ")))
...
# Remove rows with NAs (e.g. outside Reef Guide spatial coverage)
...
vip_plot <- ggplot(vip_df, aes(x = base_var, y = importance)) +
  ...
  labs(
    title = "MaxEnt (Reef Guide 17-Predictor) Variable Importance",
...
print("Predicting Reef Guide MaxEnt suitability to spatial grid...")
...
cat("\n=== Reef Guide MaxEnt Prediction Summary ===\n")
...
print("Done! Reef Guide MaxEnt model fitting and prediction complete.")
