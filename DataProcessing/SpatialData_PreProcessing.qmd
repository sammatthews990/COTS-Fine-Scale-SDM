...
for (v in rg_vars) {
  ...
  # e.g. "Cairns-Cooktown_bathy.tif", "FarNorthern_bathy.tif", ...
  files_v <- list.files(
    rg_dir,
  ...
  ## 1. Read each tile, harmonise to first tile’s grid if needed ----------
  tiles <- lapply(files_v, rast)

  base  <- tiles[[1]]  # native template for this variable

  # If any tile has slightly different geom, project it to 'base'
  tiles <- lapply(tiles, function(r) {
    if (!compareGeom(base, r, stopOnError = FALSE)) {
      project(r, base, method = if (v %in% rg_cat) "near" else "bilinear")
    } else {
      r
    }
  })

  ## 2. Merge tiles on native grid ---------------------------------------
  # Different extents are fine; merge() will take the union
  r_native <- do.call(merge, tiles)

  ## 3. Warp merged raster to 30 m template ------------------------------
  method <- if (v %in% rg_cat) "near" else "bilinear"

  out_file <- file.path(tempdir(), paste0("RG_", v, "_30m.tif"))

  r_30 <- project(
    r_native,
    y        = template30,   # your 30 m grid (env30[[1]])
    method   = method,
    filename = out_file,
    overwrite = TRUE
  )
...
prob_rast <- rast("outputs/COTS_prob_problem.tif")
plot(prob_rast)
